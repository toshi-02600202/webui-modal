# run.py
# camenduruからwgetするのでなく、変更したrun.pyを実行する。
# modal nfs put local_file remote_file
# modal nfs put run.py /content/run.py
...
async def run():
    ...
    sys.argv = shlex.split("--cors-allow-origins=* --xformers --theme dark --gradio-debug --share --skip-torch-cuda-test")
